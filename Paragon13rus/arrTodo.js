...
const list = [ { name: 'create a post',  status: STATUSES.STATUS_IN_PROGRESS, priority: PRIORITIES.PRIORITY_LOW  },
               { name: 'do the dishes',  status: STATUSES.STATUS_TO_DO, priority: PRIORITIES.PRIORITY_LOW  },
               { name: 'test', status: STATUSES.STATUS_DONE, priority: PRIORITIES.PRIORYTI_HIGH  } ];

               

function addTask(task){
    let newTask = {name: task, status: STATUSES.STATUS_TO_DO, priority: PRIORITIES.PRIORYTI_MEDIUM}
    list.push(newTask)
}
