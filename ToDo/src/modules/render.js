...
export function render(todo, high, low) {
    ...
    add.addEventListener('click', () => {
      try {
        if (add.classList.value.includes('high')) {
          addTask(todo, forms[0], high, 'check')
          render(todo, high, low)
        } else if (add.classList.value.includes('low')) {
          addTask(todo, forms[1], low, 'check')
          render(todo, high, low)
        } else {
          throw new Error('Неизвестная форма')
        }
