...
function handlerCitySearch (event) {
  ...
  let cityName = ''
  if (!UI_ELEMENTS.cityInput.value.trim()) {
    ERROR_LIST.empty_search()
    return
  }
  cityName = UI_ELEMENTS.cityInput.value
...
function renderTabNow (dataWeather) {
  ...
  const iconSrc = dataWeather.weather[0].icon
  imgCityWeather.src = `https://openweathermap.org/img/wn/${iconSrc}@4x.png`
...
function renderTabDetailes (dataWeather) {
  ...
  for (const [key, value] of dataTabDetailes) {
...
function getSunRiseAndSet (timeNeed) {
  ...
  const localTimeSun = timeNeed * 1000
  const utcSun = localTimeSun + localOffset
  const ndSun = utcSun + (dataWeather.timezone * 1000)
  const timeRequest = new Date(ndSun).toLocaleTimeString(navigator.language, FOR_DATE.optionsTime)
  return timeRequest
}
